...
class TagRepositoryImpl implements TagRepository {
  ...
  Future<Result<List<Tag>>> getTagsByJournalId(int journalId) async {
    try {
      ...
      return Result.error(Exception('일기의 태그를 가져오는데 실패했습니다: $e'));
    }
  }
  ...
  Future<Result<void>> addTagToJournal(int journalId, int tagId) async {
    try {
      ...
      if (e.extendedResultCode == 2067) {
        // UNIQUE constraint failed
        return Result.error(Exception('이미 일기에 추가된 태그입니다'));
      }

      if (e.extendedResultCode == 787) {
        // FOREIGN KEY constraint failed
        return Result.error(Exception('존재하지 않는 일기 또는 태그입니다'));
      }

      return Result.error(Exception('일기에 태그 추가에 실패했습니다: $e'));
    } catch (e) {
      return Result.error(Exception('일기에 태그 추가에 실패했습니다: $e'));
    }
  }
  ...
  Future<Result<void>> removeTagFromJournal(int journalId, int tagId) async {
    try {
      ...
      if (affectedRows == 0) {
        return Result.error(Exception('일기에서 해당 태그를 찾을 수 없습니다'));
      }

      return Result.ok(null);
    } catch (e) {
      return Result.error(Exception('일기에서 태그 제거에 실패했습니다: $e'));
    }
  }
  ...
  Future<Result<void>> updateJournalTags(
    ...
    try {
      ...
      if (e.extendedResultCode == 787) {
        // FOREIGN KEY constraint failed
        return Result.error(Exception('존재하지 않는 일기 또는 태그가 포함되어 있습니다'));
      }
      return Result.error(Exception('일기 태그 업데이트에 실패했습니다: $e'));
    } catch (e) {
      return Result.error(Exception('일기 태그 업데이트에 실패했습니다: $e'));
    }
  }
